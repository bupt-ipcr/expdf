...
def get_ref_title(ref_text, *, strict=False):
    ...
    if re.search(r'''([^“]+)[,.]?\s*“(.+)”.*(in|arxiv|doi|journal|IEEE|\wcom)''', ref_text, re.I):
        return re.search(r'''([^“]+)[,.]?\s*“(.+)”.*(in|arxiv|doi|journal|IEEE|\wcom)''', ref_text, re.I).groups()[1]
    ...
    if re.search(r'''(.+?[^A-Z])\.\s*([^.]+?[^A-Z])\.(.*)$''', ref_text): 
        ...
        if re.search(r'''in|arxiv|doi|journal|IEEE|\wcom''', groups[2], re.I):
            return groups[1]
